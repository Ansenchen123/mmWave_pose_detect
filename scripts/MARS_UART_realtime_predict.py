...
def style_3d(ax, title: str, xlim, ylim, zlim):
	ax.set_title(title, fontsize=10, fontweight='bold', pad=3)
	ax.set_xlim(*xlim)
	ax.set_ylim(*ylim)
	ax.set_zlim(*zlim)
	ax.set_xlabel('X left-right (m)', fontsize=7, labelpad=1)
	ax.set_ylabel('Y depth (m)', fontsize=7, labelpad=1)
	ax.set_zlabel('Z height (m)', fontsize=7, labelpad=1)
	ax.tick_params(labelsize=6)
	ax.view_init(elev=15, azim=-60)


def draw_point_cloud(ax, fmap: np.ndarray, frame_number: int):
	points = fmap.reshape(-1, 5)
	points = points[np.any(points != 0, axis=1)]
	if len(points) > 0:
		intensity = points[:, 4].astype(np.float32)
		p1, p99 = np.percentile(intensity, [1, 99])
		if p99 - p1 < 1e-6:
			color_value = np.zeros_like(intensity)
		else:
			color_value = np.clip((intensity - p1) / (p99 - p1), 0.0, 1.0)

		ax.scatter(
			points[:, 0], points[:, 1], points[:, 2],
			c=color_value,
			cmap=str(cfg_get(RADAR_CONFIG, 'display', 'scatter', 'cmap', default='turbo')),
			vmin=0.0,
			vmax=1.0,
			s=float(cfg_get(RADAR_CONFIG, 'display', 'scatter', 'size', default=45)),
			alpha=float(cfg_get(RADAR_CONFIG, 'display', 'scatter', 'alpha', default=0.95)),
			depthshade=False,
			edgecolors=str(cfg_get(RADAR_CONFIG, 'display', 'scatter', 'edge_color', default='k')),
			linewidths=float(cfg_get(RADAR_CONFIG, 'display', 'scatter', 'line_width', default=0.15)),
		)

	style_3d(ax, f'Radar Point Cloud\nframe {frame_number}, pts {len(points)}', PC_X, PC_Y, PC_Z)


def draw_skeleton(ax, joints: np.ndarray):
	ax.scatter(joints[:, 0], joints[:, 1], joints[:, 2], c='#c0392b', s=35, depthshade=False)
	...
	for idx, name in enumerate(JOINT_NAMES):
		ax.text(joints[idx, 0], joints[idx, 1], joints[idx, 2], name, fontsize=6, color='#7f0000')
	style_3d(ax, 'MARS Realtime Estimation', LBL_X, LBL_Y, LBL_Z)
...
class RealtimePredictViewer:
	def __init__(self):
		...
		self.fig.canvas.manager.set_window_title('MARS UART Realtime Predict')
		outer = gridspec.GridSpec(2, 1, figure=self.fig, height_ratios=[5, 0.7], hspace=0.05)
		top = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=outer[0], wspace=0.04)
		self.ax_pc = self.fig.add_subplot(top[0], projection='3d')
		self.ax_pose = self.fig.add_subplot(top[1], projection='3d')
		self.ax_text = self.fig.add_subplot(outer[1])
		self.ax_text.axis('off')
		self.angle_text = self.ax_text.text(
			0.5, 0.5, '',
	...
	def update(self, fmap: np.ndarray, pred_57: np.ndarray, frame_number: int):
		...
		self.ax_pc.cla()
		self.ax_pose.cla()
		draw_point_cloud(self.ax_pc, fmap, frame_number)
		draw_skeleton(self.ax_pose, joints)
